    ...
    <div>
        ...
        <el-row style="margin-top: 20px;">
  			<el-col :span="15" :offset="4">
	  			<el-form :model="mechanicalPreForm" ref="mechanicalPreForm" label-width="120px">
					<el-form-item>
					...
					<el-form-item label="危险程度：" class="scoreCenter">
						<span>{{mechanicalPreForm.assessmentScore}}分</span> <span>{{mechanicalPreForm.assessmentResultExplain}}</span>
					</el-form-item>
...
<script>
    ...
    export default {
    	...
    	methods: {
    		getAssessmentDictList(){
				...
				if(loginAssessmentDictPojoList!=null&&loginAssessmentDictPojoList.length>0){
					loginAssessmentDictPojoList.forEach((loginAssessmentDictTypePojo, index) => {
                        if(loginAssessmentDictTypePojo.assessmentItem == menuconstants.mechanicalPrecautionDictValue){
		    ...
		    saveInfo() {
		    	this.$refs.mechanicalPreForm.validate(async (valid) => {
					if (valid) {
						const params = {
							...this.mechanicalPreForm,
						}
						try{
							params.modelCode = menuconstants.mechanicalPrecautionModelCode;
    		...
    		async automaticCheckout(){
    			const queryVteAssessmentLastTime = await getQueryVteAssessmentLastTime({modelCode:menuconstants.mechanicalPrecautionModelCode, patientHospitId:this.$route.query.patientHospitId});
    			this.mechanicalPreForm.assessmentResult = queryVteAssessmentLastTime.assessmentResult;
    			this.mechanicalPreForm.assessmentScore = queryVteAssessmentLastTime.assessmentScore;
    			if(this.mechanicalPreForm.assessmentScore==0){
    				this.mechanicalPreForm.assessmentResult = "1";
    				this.mechanicalPreForm.assessmentResultExplain = "无";
    			}else if(this.mechanicalPreForm.assessmentScore>=1){
    				this.mechanicalPreForm.assessmentResult = "2";
    				this.mechanicalPreForm.assessmentResultExplain = "有";
    			}
    			this.dictPojoLists = [];
    		...
    		checkinlist(item){
				...
				this.mechanicalPreForm.assessmentScore = 0;
					...
					for(var score in this.assessmentDictList){
						...
						assessmentDict.forEach((assessment, index) => {
							if ((","+assessmentSelectData+",").indexOf(","+assessment.value+",")>=0) {
								this.mechanicalPreForm.assessmentScore = this.mechanicalPreForm.assessmentScore + Number(score);
							}
						});
					}
				};
    			//this.mechanicalPreForm.assessmentScore += Number(item.score);
    			if(this.mechanicalPreForm.assessmentScore==0){
    				this.mechanicalPreForm.assessmentResult = "1";
    				this.mechanicalPreForm.assessmentResultExplain = "无";
    			}else if(this.mechanicalPreForm.assessmentScore>=1){
