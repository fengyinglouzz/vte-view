    ...
    <div>
        ...
        <el-row style="margin-top: 20px;">
  			<el-col :span="15" :offset="4">
	  			<el-form :model="surgeryForm" ref="surgeryForm" label-width="150px">
					<el-form-item>
					...
					<el-form-item label="外科出血评估评分：" class="scoreCenter">
						<span>{{surgeryForm.assessmentScore}}分</span> <span>{{surgeryForm.assessmentResultExplain}}</span>
					</el-form-item>
    ...
    export default {
    	...
    	methods: {
    		getAssessmentDictList(){
				...
				if(loginAssessmentDictPojoList!=null&&loginAssessmentDictPojoList.length>0){
					loginAssessmentDictPojoList.forEach((loginAssessmentDictTypePojo, index) => {
                        if(loginAssessmentDictTypePojo.assessmentItem == menuconstants.surgeryDictValue){
		    ...
		    saveInfo() {
		    	this.$refs.surgeryForm.validate(async (valid) => {
					if (valid) {
						...
						try{
							params.modelCode = menuconstants.surgeryModelCode;
    		...
    		async automaticCheckout(){
    			const queryVteAssessmentLastTime = await getQueryVteAssessmentLastTime({modelCode:menuconstants.surgeryModelCode, patientHospitId:this.$route.query.patientHospitId});
    			this.surgeryForm.assessmentResult = queryVteAssessmentLastTime.assessmentResult;
    			this.surgeryForm.assessmentScore = queryVteAssessmentLastTime.assessmentScore;
    			if(this.surgeryForm.assessmentScore==0){
    				this.surgeryForm.assessmentResult = "1";
    				this.surgeryForm.assessmentResultExplain = "无";
    			}else if(this.surgeryForm.assessmentScore>=1){
    				this.surgeryForm.assessmentResult = "2";
    				this.surgeryForm.assessmentResultExplain = "有";
    			}
    			this.dictPojoLists = [];
    		...
    		checkinlist(item){
				...
				this.surgeryForm.assessmentScore = 0;
					...
					for(var score in this.assessmentDictList){
						...
						assessmentDict.forEach((assessment, index) => {
							if ((","+assessmentSelectData+",").indexOf(","+assessment.value+",")>=0) {
								this.surgeryForm.assessmentScore = this.surgeryForm.assessmentScore + Number(score);
							}
						});
					}
				};
    			//this.surgeryForm.assessmentScore += Number(item.score);
    			if(this.surgeryForm.assessmentScore==0){
    				this.surgeryForm.assessmentResult = "1";
    				this.surgeryForm.assessmentResultExplain = "无";
    			}else if(this.surgeryForm.assessmentScore>=1){
