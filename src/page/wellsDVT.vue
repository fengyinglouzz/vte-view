    ...
    <div>
        ...
        <el-row style="margin-top: 20px;">
  			<el-col :span="15" :offset="4">
	  			<el-form :model="wellsDVTForm" ref="wellsDVTForm" label-width="110px" class="form">
					<el-form-item>
					...
					<el-form-item label="可能性：" class="scoreCenter">
						<span>{{wellsDVTForm.assessmentScore}}分</span> <span>{{wellsDVTForm.assessmentResultExplain}}可能</span>
					</el-form-item>
					<div class="notes">注：
...
<script>
    ...
    export default {
    	...
    	methods: {
    		getAssessmentDictList(){
				...
				if(loginAssessmentDictPojoList!=null&&loginAssessmentDictPojoList.length>0){
					loginAssessmentDictPojoList.forEach((loginAssessmentDictTypePojo, index) => {
                        if(loginAssessmentDictTypePojo.assessmentItem == menuconstants.wellsDVTDictValue){
		    ...
		    saveInfo() {
		    	this.$refs.wellsDVTForm.validate(async (valid) => {
					if (valid) {
						const params = {
							...this.wellsDVTForm,
						}
						try{
							params.modelCode = menuconstants.wellsDVTModelCode;
    		...
    		async automaticCheckout(){
    			...
    			this.wellsDVTForm.assessmentResult = queryVteAssessmentLastTime.assessmentResult;
    			this.wellsDVTForm.assessmentScore = queryVteAssessmentLastTime.assessmentScore;
    			if((this.wellsDVTForm.assessmentScore<=0)){
    				this.wellsDVTForm.assessmentResult = "1";
    				this.wellsDVTForm.assessmentResultExplain = "低度";
    			}else if((this.wellsDVTForm.assessmentScore>=1) && (this.wellsDVTForm.assessmentScore<=2)){
    				this.wellsDVTForm.assessmentResult = "2";
    				this.wellsDVTForm.assessmentResultExplain = "中度";
    			}else if((this.wellsDVTForm.assessmentScore>=3)){
    				this.wellsDVTForm.assessmentResult = "3";
    				this.wellsDVTForm.assessmentResultExplain = "高度";
    			}
    			this.dictPojoLists = [];
    		...
    		async notesTable(){
    			const queryVteRiskScoreInfoList = await getQueryVteRiskScoreInfoList({modelCode:menuconstants.wellsDVTModelCode});
				this.notesData=[];
    		...
    		checkinlist(item){
				...
				this.wellsDVTForm.assessmentScore = 0;
					...
					for(var score in this.assessmentDictList){
						...
						assessmentDict.forEach((assessment, index) => {
							if ((","+assessmentSelectData+",").indexOf(","+assessment.value+",")>=0) {
								this.wellsDVTForm.assessmentScore = this.wellsDVTForm.assessmentScore + Number(score);
							}
						});
					}
				};
    			//this.wellsDVTForm.assessmentScore += Number(item.score);
    			if((this.wellsDVTForm.assessmentScore<=0)){
    				this.wellsDVTForm.assessmentResult = "1";
    				this.wellsDVTForm.assessmentResultExplain = "低度";
    			}else if((this.wellsDVTForm.assessmentScore>=1) && (this.wellsDVTForm.assessmentScore<=2)){
