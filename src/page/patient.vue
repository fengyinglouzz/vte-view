<template>
	<div class="patient fillcontain" style="height: 60px;background-color: #324057;">
		<el-menu :default-active="defaultActive" mode="horizontal" theme="dark" router >
			<div  v-for="(item, index) in  $router.options.routes[2].children" :key="index" >
    	...
    	export default {
			...
			methods: {
				refreshrouter(){
					 ...
					 router.options.routes[2].children.forEach((item, index) => {
						  var menuShow = false;
							loginModelPojoList.forEach((itemModel, index) => {
									const key = "M"+itemModel.modelCode.replace("-","_");
