    ...
    <div>
        ...
        <el-row style="margin-top: 20px;">
  			<el-col :span="15" :offset="4">
	  			<el-form :model="paduaForm" ref="paduaForm" label-width="110px">
					<el-form-item label="阶段">
						<el-select v-model="paduaForm.assessmentStage" clearable placeholder="请选择" style="width:100%;">
							<el-option
					...
					<el-col :span="8" v-for="(dictPojoList,index) in assessmentDictList" :key="index">
					...
					<el-form-item label="Padua评分：" class="scoreCenter">
						<span>{{paduaForm.assessmentScore}}分</span> <span>{{paduaForm.assessmentResultExplain}}</span>
					</el-form-item>
					<div class="notes">注：
...
<script>
    ...
    export default {
    	...
    	methods: {
    		...
    		getAssessmentDictList(){
				...
				if(loginAssessmentDictPojoList!=null&&loginAssessmentDictPojoList.length>0){
					loginAssessmentDictPojoList.forEach((loginAssessmentDictTypePojo, index) => {
                        if(loginAssessmentDictTypePojo.assessmentItem == menuconstants.paduaDictValue){
		    ...
		    saveInfo() {
		    	this.$refs.paduaForm.validate(async (valid) => {
					if (valid) {
						const params = {
							...this.paduaForm,
						}
						try{
							params.modelCode = menuconstants.paduaModelCode;
    		...
    		async automaticCheckout(){
    			const queryVteAssessmentLastTime = await getQueryVteAssessmentLastTime({modelCode:menuconstants.paduaModelCode, patientHospitId:this.$route.query.patientHospitId});
    			this.paduaForm.assessmentResult = queryVteAssessmentLastTime.assessmentResult;
    			this.paduaForm.assessmentScore = queryVteAssessmentLastTime.assessmentScore;
    			if((this.paduaForm.assessmentScore>=0) && (this.paduaForm.assessmentScore<4)){
    				this.paduaForm.assessmentResult = "1";
    				this.paduaForm.assessmentResultExplain = "低危";
    			}else if(this.paduaForm.assessmentScore>=4){
    				this.paduaForm.assessmentResult = "3";
    				this.paduaForm.assessmentResultExplain = "高危";
    			}
    			this.dictPojoLists = [];
    		...
    		async notesTable(){
    			const queryVteRiskScoreInfoList = await getQueryVteRiskScoreInfoList({modelCode:menuconstants.paduaModelCode});
				this.notesData=[];
                  ...
                  const notesData = {};
                    ...
                    this.notesData.riskScoreRate = Number((item.riskScoreRate)*100)+"%";
                });
    		},
    		ifendcase( val ){
    		...
    		checkinlist(item){
				...
				this.paduaForm.assessmentScore = 0;
					...
					for(var score in this.assessmentDictList){
						...
						assessmentDict.forEach((assessment, index) => {
							if ((","+assessmentSelectData+",").indexOf(","+assessment.value+",")>=0) {
								this.paduaForm.assessmentScore = this.paduaForm.assessmentScore + Number(score);
							}
						});
					}
				};
    			//this.paduaForm.assessmentScore += Number(item.score);
    			if((this.paduaForm.assessmentScore>=0) && (this.paduaForm.assessmentScore<4)){
    				this.paduaForm.assessmentResult = "1";
    				this.paduaForm.assessmentResultExplain = "低危";
    			}else if(this.paduaForm.assessmentScore>=4){
